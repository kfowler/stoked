...
% ── Abstract ──────────────────────────────────────────────────────────────────
\begin{abstract}
We introduce \stoked{} (\textbf{S}tochastic \textbf{T}yped \textbf{O}perations \textbf{K}it for \textbf{E}ngineering \textbf{D}elivery), a formal specification language for describing production systems in which workstations are LLM-based agents, deterministic computations, or human task queues, and jobs are software artifacts flowing through a queueing network.
...
\begin{equation}\label{eq:vut}
  W_q \;\approx\; \underbrace{\left(\frac{\scv_a + \scv_s}{2}\right)}_{\text{Variability}} \;\cdot\; \underbrace{\frac{\rho^{\sqrt{2(c+1)}}}{c\,(1 - \rho)}}_{\text{Utilization}} \;\cdot\; \underbrace{E[S]}_{\text{Time}}
\end{equation}
...
A 4-station pipeline (Build $\to$ Code Review $\to$ Test $\to$ Deploy) with a rework loop (30\% of reviews request changes).
